...
const Menu = () => {
    const { data, loading, error } = useFetch("https://awamp-api.vercel.app/plantas")
    const { data: user, loading: loadingUser, error: errorUser } = useFetch('https://awamp-api.vercel.app/usuarios/jorge@test.com')

    const { email, setEmail } = useContext(MyEstadoGlobalContext)
    const [searchBox, setSearchBox] = useState('')

    if (errorUser) alert(errorUser)
    ...
    return (
        <div className='flex flex-col flex-wrap justify-center overflow-auto items-center min-h-screen bg-black text-neutral-50 bg-fixed bg-cover py-5'>
            <div className='bg-neutral-600 p-8 rounded-lg flex flex-col justify-between gap-4 items-center w-[70vw]'>
                ...
                <button className='bg-neutral-400 p-2 rounded-lg text-center hover:bg-neutral-300 transition duration-200 ease-in-out'>
                    <Link to={"/Add"} state={{ detail: "Some detail" }}>Add</Link>
                </button>
                <FormTable />
